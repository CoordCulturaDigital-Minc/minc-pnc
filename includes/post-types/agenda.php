...
class Agenda {
    ...
    static function register(){
        register_post_type('agenda', array(
                ...
                'labels' => array(
                                    'name' => _x('Evento', 'post type general name', 'minc'),
                                    'singular_name' => _x('Evento', 'post type singular name', 'minc'),
                                    'add_new' => _x('Adicionar novo', 'image', 'minc'),
                                    'add_new_item' => __('Adicionar novo evento', 'minc'),
                                    'edit_item' => __('Editar evento', 'minc'),
                                    'new_item' => __('Novo evento', 'minc'),
                                    'view_item' => __('Ver evento', 'minc'),
                                    'search_items' => __('Buscar eventos', 'minc'),
                                    'not_found' =>  __('Nenhum evento encontrado', 'minc'),
                                    'not_found_in_trash' => __('Nenhum evento encontrado na lixeira', 'minc'),
                                    'parent_item_colon' => ''
                                 ),
                 'public' => true,
                 'rewrite' => true,
...
add_action('pre_get_posts', 'minc_agenda_query');

function minc_agenda_query($wp_query) {
...
add_action('admin_init', function() {
    ...
    wp_enqueue_style('ui-lightness', get_bloginfo('template_url') . '/css/ui-lightness/jquery-ui-1.8.21.custom.css');
});
...
function the_event_box() {
    ...
    if (is_array($meta) && !empty($meta)) {
        ...
        <div class="event-info clear">
            ...
            <?php
            if ($meta['_data_inicial'][0]) echo '<p class="bottom"><strong>Data Inicial:</strong> ', date('d/m/Y', strtotime($meta['_data_inicial'][0])), '</p>';
            if ($meta['_data_final'][0]) echo '<p class="bottom top"><strong>Data Final:</strong> ', date('d/m/Y', strtotime($meta['_data_final'][0])), '</p>';
            if ($meta['_horario'][0]) echo '<p class="bottom top"><strong>Horário:</strong> ', $meta['_horario'][0], '</p>';
            if ($meta['_onde'][0]) echo '<p class="bottom top"><strong>Local:</strong> ', $meta['_onde'][0], '</p>';
            if ($meta['_link'][0]) echo '<p class="bottom top"><strong>Site:</strong> ', $meta['_link'][0], '</p>';
            ?>
        </div>
        <?php
    }
